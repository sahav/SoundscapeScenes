...
gdrive = "\\acoustic\\data\\users\\haver\\MANTA\\"
dirOut =  paste0( gdrive, "SoundCoop_AcousticScene\\CombineData\\A_outputHMDDETS")
...
if (siteN == "AU_CH01") {
  ...
  inDir   = "\\acoustic\\data\\users\\haver\\MANTA\\manta_data\\output\\NRS01\\..
   ...NRS_01_20180820-20200824_HMD\\MILLIDEC_MinRes\\2018\\ch_NRS01_H5R6B.1.5000\\"
  inFiles = list.files( inDir, pattern = "MinRes", full.names = T, recursive = T)
} else if (siteN == "NRS11"){
  inDir   = paste0("Z:\\MANTA\\pypam\\", siteN, "\\NetCDF\\" )
  inFiles = list.files( inDir, pattern = "*.nc", full.names = T, recursive = F)
  inDirs=inDir
  dirOut = "Z:\\MANTA\\pypam\\Scenes"
...
#inFiles = list.files( inDir[yy], pattern = "*.nc", full.names = T, recursive = F)
