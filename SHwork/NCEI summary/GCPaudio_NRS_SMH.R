...
output2 <- within(
  output2, {
    Location <- as.factor(
      ifelse(SiteName %in% c('01', '02','04','06','07','08'), 'NMFS', 
             ifelse(SiteName %in% c('03', '05', '09',11), 'NMS', 
                    ifelse(SiteName %in% c(10,12), 'NPS', NA) 
                    )
             )
      )
  })
...
pT = ggplot(outputT, aes(y = Site, x = Start_Date, xend = End_Date)) +
  ...
  #scale_y_discrete(labels = loc_names_rev,limits=rev) +
  theme_minimal() +
...
# Gantt for paper
loc_names =c("Alaskan Arctic", "Gulf of Alaska", "Olympic Coast NMS", "Hawai’i","Channel Islands NMS","Gulf of Mexico","S.E. U.S.", "N.E. U.S.",
             "Stellwagen Bank NMS","American Samoa (NPS)","Cordell Bank NMS", "U.S. V.I. (NPS)")
loc_names_rev =c("U.S.V.I. (NPS)","Cordell Bank NMS","American Samoa (NPS)","Stellwagen Bank NMS", "N.E. U.S.","S.E. U.S.","Gulf of Mexico",
              "Channel Islands NMS","Hawai’i","Olympic Coast NMS","Gulf of Alaska","Alaskan Arctic")

pT = ggplot(outputT, aes(y = Site, x = Start_Date, xend = End_Date)) +
  geom_tile(aes(x = Start_Date, width = as.numeric(End_Date - Start_Date), fill = Location, 
           height = 0.8))+  # Fill color by Instrument and outline in black
  scale_fill_manual( values = loc_colors) +  # Use specific colors for instruments
  labs(x = "", y = "", title = paste0(toupper(projectN),  " - summary of archived data")) +
       #subtitle = paste0("NCEI google cloud platform (", typ, ")")) +
  scale_y_discrete(labels = loc_names_rev,limits=rev) +
  theme_minimal() + 
...
pT
fileName = paste0(outputDir, "\\", projectN, "_dataGantt_SH", DC )
save(outputT, file = paste0( fileName, ".Rda") )
write.csv(outputT, file = paste0( fileName, ".csv") )
ggsave(filename = paste0( fileName, ".png"), plot = pT, width = 8, height = 6, dpi = 600)
